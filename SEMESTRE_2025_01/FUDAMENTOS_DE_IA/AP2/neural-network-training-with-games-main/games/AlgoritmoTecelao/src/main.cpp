...
PIG_Cor getPixelColor(int objeto, long coluna, long linha, int largura, int altura)
{
    PIG_Cor cor;
...
void somarColor(int objeto, long coluna, long linha, int largura, int altura, int coef)
{
...
double somarLinha(int Imagem, int Inicio, int Final)
{
    ...
    for (double i = 0; i <= Distancia; i++)
    {
        long X = roundl(Pregos[Inicio].X + (i * (dx / Distancia)));
        long Y = roundl((Pregos[Inicio].Y + (i * (dy / Distancia))));
...
void desenhar()
{
    ...
    {
        sprintf(String, "Fios tracados: %d de %d", contador, QTD_ITERACAO);
    }
    else
    {
    ...
    if (janelaSemFoco == 1)
    {
        ...
        EscreverCentralizada(String, 700, 300, FonteVermelha);
    }
    EncerrarDesenho();
}
...
int main(int argc, char *args[])
{
    ...
    Quadro = criarObjeto("imagens/branco.bmp", 0, 0, ALT_IMG, LARG_IMG, 0);
