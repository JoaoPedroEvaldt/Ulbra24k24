...
int buscarMelhorFitness() {
    ...
    if (indice < 0 || indice >= QTD_CARROS) {
        printf("ERRO: índice inválido em buscarMelhorFitness()\n");
        return 0; // Retorna o primeiro carro como fallback
    }
...
void inicializarSprites()
{
    ...
    spriteTurbo = CriarSprite("imagens/turbo.png",0);
...
{
    if (i < 0 || i >= QTD_CARROS) {
        printf("Índice inválido em alocarCarro: %d\n", i);
        return;
    }

    carros[i].Sprite = spriteCarros[rand()%QTD_SPRITES_CARROS];
    carros[i].Cerebro = RNA_CriarRedeNeural(
    ...
    if (!carros[i].Cerebro) {
        printf("Falha ao criar rede neural para carro %d\n", i);
        exit(1);
    }
...
void salvarRedeArquivo() {
    char String[1000];
    int indice = buscarMelhorFitness(); // Verifique se retorna um índice válido (0 a QTD_CARROS-1)

    // Adicione esta verificação:
    if (indice < 0 || indice >= QTD_CARROS) {
        printf("Índice inválido em buscarMelhorFitness(): %d\n", indice);
        return;
    }
    ...
    // Verifique se o diretório "redes" existe:
    system("mkdir -p redes"); // Cria a pasta se não existir
    ...
    fwrite(&carros[indice].TamanhoDNA, sizeof(int), 1, f);
    fwrite(carros[indice].DNA, sizeof(double), carros[indice].TamanhoDNA, f);
...
int main(int argc, char* args[]) {
    ...
    while (1) {  // Loop infinito para o jogo continuar rodando
        while (PIG_JogoRodando == 1) {  // Loop interno para cada geração
            AtualizarJanela();
        ...
        // Se saiu do loop interno (PIG_JogoRodando == 0), mas queremos continuar
        if (carrosColididos == QTD_CARROS) {
            encerrarPartida();
            randomMutations();
            inicializarNovaPartida();
            PIG_JogoRodando = 1;  // Reinicia o jogo para a próxima geração
        } else {
            break;  // Sai do loop se o jogo foi fechado pelo usuário
        }
    }
